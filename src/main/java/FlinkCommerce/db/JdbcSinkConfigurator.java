...
public class JdbcSinkConfigurator<T> {
    ...
    public JdbcSinkConfigurator(String jdbcUrl, String username, String password,DataStream<Transaction>  transactionStream) {
        ...
        this.connOptions = new JdbcConnectionOptions.JdbcConnectionOptionsBuilder()
                .withUrl(jdbcUrl)
        ...
        transactionStream.map(
                        transaction -> {
                            Date transactionDate = new Date(System.currentTimeMillis());
                            double totalSales = transaction.getTotalAmount();
                            return new JSONValueDeserializationSchema.SalesPerDay(transactionDate, totalSales);
                        }
                ).keyBy(JSONValueDeserializationSchema.SalesPerDay::getTransactionDate)
                .reduce((salesPerDay, t1) -> {
                    ...
                    return salesPerDay;
                        ...
                        (JdbcStatementBuilder<JSONValueDeserializationSchema.SalesPerDay>) (preparedStatement, salesPerDay) -> {
                            preparedStatement.setDate(1, new Date(System.currentTimeMillis()));
        ...
        transactionStream.map(
                        transaction -> {
                            ...
                            double totalSales = transaction.getTotalAmount();
                            return new JSONValueDeserializationSchema.SalesPerMonth(year, month, totalSales);
                        }
                ).keyBy(JSONValueDeserializationSchema.SalesPerMonth::getMonth)
                .reduce((salesPerMonth, t1) -> {
                    ...
                    return salesPerMonth;
                        ...
                        (JdbcStatementBuilder<JSONValueDeserializationSchema.SalesPerMonth>) (preparedStatement, salesPerMonth) -> {
